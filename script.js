...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Safeguard: if any element is missing, show error in console and stop further JS that depends on it
  if(!searchInput || !cardsContainer || !paginationEl || !statusEl || !minMontoInput || !maxMontoInput){
    console.error("Elemento(s) DOM faltante(s). Verifica el HTML y los IDs.");
    statusEl && (statusEl.textContent = "Error: falta algún elemento del DOM. Revisa la consola.");
    return;
  }
  ...
  function openModal(item){
    ...
    if(cel){
      ...
      if(link) link.href = `https://wa.me/51${cel}`;
    } else {
